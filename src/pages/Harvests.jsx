import { useEffect, useState } from "react";
import { api } from "../api/axios";

export default function Harvests() {
    const [harvests, setHarvests] = useState([]);
    const [isOpen, setIsOpen] = useState(false);
    const [editId, setEditId] = useState(null);
    const [fields, setFields] = useState([]);
    const [crops, setCrops] = useState([]);

    const [form, setForm] = useState({
        crop_id: "",
        field_id: "",
        harvest_date: "",
        quantity_kg: "",
        quality_grade: "",
        price_per_kg: ""
    });

    useEffect(() => {
        api.get("/harvests").then(res => setHarvests(res.data));

        api.get("/fields").then(res => setFields(res.data));
        api.get("/crops").then(res => setCrops(res.data));
    }, []);

    const fetchData = () => {
        api.get("/harvests")
            .then(res => setHarvests(res.data))
            .catch(err => console.log(err));
    };

    const saveHarvest = async () => {
        try {
            const payload = {
                ...form,
                crop_id: Number(form.crop_id),
                field_id: Number(form.field_id),
                quantity_kg: Number(form.quantity_kg),
                price_per_kg: Number(form.price_per_kg)
            };

            if (editId) {
                await api.put(`/harvests/${editId}`, payload);
            } else {
                await api.post("/harvests", payload);
            }

            fetchData();
            setIsOpen(false);
            setEditId(null);

            setForm({
                crop_id: "",
                field_id: "",
                harvest_date: "",
                quantity_kg: "",
                quality_grade: "",
                price_per_kg: ""
            });

        } catch (err) {
            console.log(err);
        }
    };

    return (
        <div>
            <div className="flex justify-between items-center mb-6">
                <h1 className="text-2xl font-bold">Harvest Records</h1>

                <button
                    onClick={() => {
                        setIsOpen(true);
                        setEditId(null);
                        setForm({
                            crop_id: "",
                            field_id: "",
                            harvest_date: "",
                            quantity_kg: "",
                            quality_grade: "",
                            price_per_kg: ""
                        });
                    }}
                    className="bg-green-500 text-white px-4 py-2 rounded"
                >
                    + Add Harvest
                </button>
            </div>
            <div className="bg-white rounded-xl shadow overflow-hidden">
                <div className="overflow-auto max-h-[85vh]">
                    <table className="w-full text-sm">
                        <thead className="bg-gray-100 text-left">
                            <tr>
                                <th className="p-3">Crop</th>
                                <th className="p-3">Field</th>
                                <th className="p-3">Date</th>
                                <th className="p-3">Kg</th>
                                <th className="p-3">Grade</th>
                                <th className="p-3">Price/kg</th>
                                <th className="p-3">Actions</th>
                            </tr>
                        </thead>

                        <tbody>
                            {harvests.map(item => (
                                <tr key={item.harvest_id} className="border-t">

                                    <td className="p-3">{item.crop_id}</td>
                                    <td className="p-3">{item.field_id}</td>

                                    <td className="p-3">
                                        {new Date(item.harvest_date).toLocaleDateString()}
                                    </td>

                                    <td className="p-3">{item.quantity_kg}</td>
                                    <td className="p-3">{item.quality_grade}</td>
                                    <td className="p-3">${item.price_per_kg}</td>

                                    <td className="p-3 flex gap-2">
                                        <button
                                            onClick={() => {
                                                setIsOpen(true);
                                                setEditId(item.harvest_id);
                                                setForm(item);
                                            }}
                                            className="px-3 py-1 border border-blue-300 text-blue-600 rounded-lg hover:bg-blue-50 transition"
                                        >
                                            Edit
                                        </button>

                                        <button
                                            onClick={async () => {
                                                await api.delete(`/harvests/${item.harvest_id}`);
                                                fetchData();
                                            }}
                                            className="px-3 py-1 border border-red-300 text-red-600 rounded-lg hover:bg-red-50 transition"
                                        >
                                            Delete
                                        </button>
                                    </td>

                                </tr>
                            ))}
                        </tbody>
                    </table>
                </div>
            </div>

            {isOpen && (
                <div className="fixed inset-0 bg-black/50 flex items-center justify-center">

                    <div className="bg-white w-[420px] p-6 rounded-xl">

                        <h2 className="text-xl font-bold mb-4">
                            {editId ? "Edit Harvest" : "Create Harvest"}
                        </h2>

                        <div className="flex flex-col gap-3">

                            <select
                                className="border p-2 rounded"
                                value={form.crop_id}
                                onChange={(e) => setForm({ ...form, crop_id: Number(e.target.value) })}
                            >
                                <option value="">Select crop</option>
                                {crops.map(c => (
                                    <option key={c.crop_id} value={c.crop_id}>
                                        {c.crop_name}
                                    </option>
                                ))}
                            </select>

                            <select
                                className="border p-2 rounded"
                                value={form.field_id}
                                onChange={(e) => setForm({ ...form, field_id: Number(e.target.value) })}
                            >
                                <option value="">Select field</option>
                                {fields.map(f => (
                                    <option key={f.field_id} value={f.field_id}>
                                        Field #{f.field_id}
                                    </option>
                                ))}
                            </select>

                            <input
                                className="border p-2 rounded"
                                type="date"
                                value={form.harvest_date}
                                onChange={(e) => setForm({ ...form, harvest_date: e.target.value })}
                            />

                            <input
                                className="border p-2 rounded"
                                placeholder="Quantity (kg)"
                                value={form.quantity_kg}
                                onChange={(e) => setForm({ ...form, quantity_kg: e.target.value })}
                            />

                            <select
                                className="border p-2 rounded"
                                value={form.quality_grade}
                                onChange={(e) => setForm({ ...form, quality_grade: e.target.value })}
                            >
                                <option value="">Select grade</option>
                                <option value="A">A — Premium</option>
                                <option value="B">B — Good</option>
                                <option value="C">C — Medium</option>
                                <option value="D">D — Low</option>
                            </select>

                            <input
                                className="border p-2 rounded"
                                placeholder="Price per kg"
                                value={form.price_per_kg}
                                onChange={(e) => setForm({ ...form, price_per_kg: e.target.value })}
                            />

                        </div>

                        <div className="flex justify-end gap-2 mt-5">

                            <button
                                onClick={() => setIsOpen(false)}
                                className="px-4 py-2 bg-gray-200 rounded"
                            >
                                Cancel
                            </button>

                            <button
                                onClick={saveHarvest}
                                className="px-4 py-2 bg-green-500 text-white rounded"
                            >
                                {editId ? "Update" : "Save"}
                            </button>

                        </div>

                    </div>
                </div>
            )}

        </div>
    );
}